...
PER_ORG = {  # doc id -> {org: (code, text, none-kind)}
 "A/300": {"TTA": ("partial", "Umbrella TTAK.KO-07.0147 (cites A/300:2020) splits the system into Parts 1–6; Part 1 (0148/R1) adds national service/system requirements (§5: 수신 신호 품질, 계층적 미디어, 재난방송 채널 자동 변경, 이종망 연동, 콘텐츠보호, 3DTV; §6 system requirements) that A/300 does not contain"), "SBTVD": ("review", "System specified as the ABNT NBR 25601–25609 series; no umbrella document")},
 "A/321": {"TTA": ("same", "TTA §10 is a translation of A/321:2016 §4–6: bootstrap structure and major-version-0 signaling identical; 6 MHz is one of the defined values. A/321:2026 additions (major version 1, heterogeneous-waveform multiplexing) not yet carried over — no effect on ATSC 3.0 services"), "SBTVD": ("same", "A/321:2025-07 incorporated by reference (Anatel Ato 2.705/2026 item 4.1); identical for ATSC 3.0 frames; 2026-04 Amd 1 not yet referenced; ABNT NBR 25601 profile unverified")},
 "A/322": {"TTA": ("same", "Physical layer identical: TTA §5–9/Annex I-1 translate A/322 (2020 text + 2024 MIMO extension) — FEC, constellations, FFT/GI/pilots, frequency interleaver, PLP/LDM; 6 MHz selected; MIMO/LDM optional. Open item: Table Ⅰ.10-5 still lists MP4_2 at 32K/GI6_1536 (removed by A/322:2025-07a Amd 1, Jan 2026)"), "SBTVD": ("same", "A/322:2025-07a incorporated by reference (Anatel Ato 2.705/2026): 6 MHz; MIMO, LDM and TxID mandatory (Decreto 12.595/2025 Art. 3º). Open item: referenced edition predates the January 2026 erratum and the June 2026 bootstrap clarification; ABNT profile unverified")},
 "A/323": {"TTA": ("none", "", "not-found"), "SBTVD": ("none", "", "not-found")},
 "A/330": {"TTA": ("same", "Part 3 §6 (6.1 개요, 6.2 ALP 패킷 포맷, 6.3 IP 헤더 압축, 6.4 링크 계층 시그널링) adopts A/330 (cited 2017 edition). Section diff A/330:2019a → 2026-04 shows only small edits (§5.4 MPEG-2 TS encapsulation, §6.1 ROHC pre-processing) — not yet itemized against the Korean text"), "SBTVD": ("review", "ABNT NBR 25602 transport layer not read")},
 "A/332": {"TTA": ("partial", "Part 2 §8 adopts A/332 (2016 Proposed Standard): data model, encapsulation, delivery, update/management; A/332 §5.6 (SG data update and management) and §5.5 (delivery) were substantially revised by 2026-08 — Korean text follows the 2016 version; Korean SG XML schema annex; also referenced by IBB Part 5"), "SBTVD": ("review", "ESG provisions to be located in ABNT NBR 25602; OG-08 §8.5–8.6 EPG/ECG in the application catalog")},
 "A/342 Part 1": {"TTA": ("same", "Part 2 §6 adopts A/342 (2016 CS): audio signal format (channels/objects/HOA, 16 core channels, 48 kHz), metadata and signaling clauses correspond to the common elements"), "SBTVD": ("review", "ABNT NBR 25604 not read")},
 "A/324": {"TTA": ("partial", "Only DSTP (§7) and ALPTP (§8) are translated (Part 4 부록 Ⅰ-2, from the 2020 candidate revision); scheduler/BGW management, CTP, STLTP and transmitter requirements are not standardized in Korea — TxID/BSID operation covered by TTAR-07.0026/R1, SFN tolerance by regulation"), "SBTVD": ("same", "A/324:2025-07 incorporated by reference (Anatel Ato 2.705/2026 item 4.1); the 2026-04 baseline differs only by reference updates (no technical change since 2025-04); TxID mandatory by decree")},
 "A/331": {"TTA": ("partial", "Signaling is essentially the same (2017-based text): LLS/SLT/SystemTime/AEAT/SLS/delivery identical in structure. National adaptations: LLS tables VIT 0x80 and CPT 0x81; serviceCategory 10 (Data only) and 5 (EAS) kept; RRT/content advisories not used; AEAVoiceText and national event/location code tables; USD_EX service-linkage extension and Korean DWD namespace; SignedMultiTable, AEA@category, RSAT, DRM-for-MMTP absent"), "SBTVD": ("review", "ABNT NBR 25602 transport layer not read (ABNT clause unverified); emergency alerting profile known from OG-07 (CAP→AEA, COBRADE, ZIP_BR)")},
 "A/341": {"TTA": ("same", "Part 2 §5 adopts A/341 (2016 CS text): HEVC Main 10 / Scalable Main 10, Main tier, Level 5.2 (also mandated by MSIT Notice Art. 13 ①3); Korean profile selects progressive 720p/1080p/2160p only (no SD/interlaced), BT.709/BT.2020, 4:2:0, 8/10-bit, 23.976–120 Hz; scalable-coding and HFR temporal-layering constraints identical (Korea requires temporal layering for 2160p120)"), "SBTVD": ("diff", "VVC (+LCEVC) adopted instead of HEVC (ABNT NBR 25603)")},
 ...
 "A/342 Part 3": {"TTA": ("same", "Part 2 §6 adopts A/342-3 (2016 CS text): MPEG-H 3D Audio LC profile Level 1–3 selected (A/342-3 also defines the Baseline profile), 48 kHz, up to 16 core channels, MHAS packet rules, mhm1/mhm2 ISOBMFF encapsulation; MSIT Notice Art. 13 ①4 mandates the same"), "SBTVD": ("partial", "MPEG-H adopted (ABNT NBR 25604; OG-04 profiles 2.0/5.1 …); profile constraints unverified")},
 "A/343": {"TTA": ("partial", "Part 2 §7 adopts A/343 (2016 CS): IMSC1 captions, ISOBMFF packaging, signaling; Korean extension: animated-image (APNG) captions (§7.2, §7.2.1); CTA-708 legacy conversion not applicable"), "SBTVD": ("review", "ABNT NBR 25605 closed captioning — clause comparison pending; closed signing (ABNT NBR 25606) is Brazil-only")},
 "A/344": {"TTA": ("diff", "HbbTV 2.0-based IBB (TTAK.KO-07.0128/R3: ETSI TS 102 796 / TS 102 809, OIPF, HTML5) instead of the A/344 Broadcaster Application — different runtime, signaling (AIT vs HELD) and APIs"), "SBTVD": ("diff", "Ginga-based application coding (ABNT NBR 25608; OG-08: NCL 4.0, NCLua, WebServices APIs, BALD signaling, application catalog) instead of A/344")},
 "A/337": {"TTA": ("partial", "IBB Part 2 carries broadcast events like A/337 — static events in the MPD Period EventStream, dynamic events in DASH 'emsg' boxes — while application signaling itself uses the AST (XML AIT-based) instead of HELD"), "SBTVD": ("review", "Ginga event handling (OG-08) — equivalence not established")},
 "A/338": {"TTA": ("partial", "IBB Part 4 companion-screen service on the HbbTV 2.0/OIPF framework; A/338 listed as a reference"), "SBTVD": ("diff", "Ginga remote-device APIs (OG-08 §9, Annex C.6.15)")},
 ...
 "A/361": {"TTA": ("diff", "Korean content protection (Part 5, SimulCrypt/CENC basis) does not follow the A/360/A/361 security model"), "SBTVD": ("none", "", "not-found")},
 "A/362": {"TTA": ("diff", "DRM handled by the Korean Part 5 mechanism (CENC, UHDCP/DP, CA Token download platform) rather than A/362"), "SBTVD": ("none", "", "not-found")},
 "A/381": {"TTA": ("diff", "Interactive content in Korea follows the HbbTV-based IBB standard, not A/344/A/381"), "SBTVD": ("diff", "Ginga application guideline (OG-08) instead of A/344/A/381")},
}
RELS = {  # doc id -> {org: (relation, confidence)}
 ...
 "A/324": {"TTA": ("profile", "confirmed"), "SBTVD": ("incorporated", "confirmed")},
 "A/337": {"TTA": ("alternative", "metadata"), "SBTVD": ("related", "metadata")},
 "A/338": {"TTA": ("related", "metadata"), "SBTVD": ("alternative", "metadata")},
 "A/331": {"TTA": ("profile", "confirmed"), "SBTVD": ("related", "metadata")},
 "A/341": {"TTA": ("profile", "confirmed"), "SBTVD": ("alternative", "confirmed")},
 ...
 "A/342 Part 3": {"TTA": ("profile", "confirmed"), "SBTVD": ("profile", "metadata")},
 "A/343": {"TTA": ("profile", "confirmed"), "SBTVD": ("related", "metadata")},
 "A/344": {"TTA": ("alternative", "confirmed"), "SBTVD": ("alternative", "confirmed")},
 "A/345": {"TTA": ("alternative", "confirmed"), "SBTVD": ("related", "metadata")},
 "A/360": {"TTA": ("alternative", "partial"), "SBTVD": ("related", "unverified")},
 "A/361": {"TTA": ("alternative", "metadata")}, "A/362": {"TTA": ("alternative", "metadata")},
 "A/330": {"TTA": ("profile", "partial"), "SBTVD": ("related", "unverified")},
 "A/332": {"TTA": ("profile", "partial"), "SBTVD": ("related", "unverified")},
 "A/381": {"TTA": ("alternative", "metadata"), "SBTVD": ("alternative", "metadata")},
 "A/300": {"TTA": ("profile", "partial"), "SBTVD": ("related", "metadata")},
}
...
data = {
  ...
  "verdicts": {
    "same":    {"icon": "✓", "label": "Same", "desc": "Clause- and table-level comparison done: no relevant difference. Choosing one of the values or options the ATSC document defines (e.g. 6 MHz, optional tools) still counts as Same — the requirement level (optional / mandatory) is shown as a tag"},
    "partial": {"icon": "△", "label": "Partial match", "desc": "Core is the same but the content differs: missing or stale clauses, errata not reflected, or constraints outside the ATSC-defined options"},
